...
#include "sensors/amcl_point_cloud.h"
...
class AmclNode
{
  ...
  private:
    ...
    std::vector< AMCLPointCloud* > clouds_;
    // std::vector< bool > lasers_update_;
    std::vector< bool > clouds_update_;
    // std::map< std::string, int > frame_to_laser_;
    std::map< std::string, int > frame_to_cloud_;
    ...
    ros::Timer check_cloud_timer_;

    int /*max_beams_, */min_particles_, max_particles_;
    double alpha1_, alpha2_, alpha3_, alpha4_, alpha5_;
    double alpha_slow_, alpha_fast_;
    // double z_hit_, z_short_, z_max_, z_rand_, sigma_hit_, lambda_short_;
    double z_hit_, z_rand_, z_sigma_;
  //beam skip related params
    ...
    ros::Time last_cloud_received_ts_;
        ...
        // first_reconfigure_call_(true)
{
  ...
  private_nh_.param("cloud_z_hit", z_hit_, 2.0);
  ...
  private_nh_.param("cloud_z_rand", z_rand_, 0.05);
  // private_nh_.param("laser_sigma_hit", sigma_hit_, 0.2);
  private_nh_.param("cloud_z_sigma", z_sigma_, 0.2);
  ...
  cloud_check_interval_ = ros::Duration(15.0);
...
//   delete cloud_;
//   cloud_ = new AMCLCloud(map_);
//   ROS_ASSERT(cloud_);
//   cloud_->SetElevationDifferenceMapModel(z_hit_, z_rand_, z_sigma_);
//   // ROS_ASSERT(laser_);
...
// 
//   initial_pose_sub_ = nh_.subscribe("initialpose", 2, &AmclNode::initialPoseReceived, this);
//   point_cloud_sub_ = nh_.subscribe("/hokuyo3d/hokuyo_cloud", 100, &AmclNode::cloudReceived, this);
// }
...
void 
AmclNode::checkCloudReceived(const ros::TimerEvent& event)
{
  ros::Duration d = ros::Time::now() - last_cloud_received_ts_;
  if(d > cloud_check_interval_)
  {
...
AmclNode::handleMapMessage(const nav_msgs::OccupancyGrid& msg)
{
  ...
  // map, #5202.
  clouds_.clear();
  clouds_update_.clear();
  frame_to_cloud_.clear();
  ...
  delete cloud_;
  cloud_ = new AMCLPointCloud(map_);
  ROS_ASSERT(cloud_);
  cloud_->SetElevationDifferenceMapModel(z_hit_, z_rand_, z_sigma_);
...
AmclNode::cloudReceived(const sensor_msgs::PointCloudConstPtr& point_cloud)
{
  ...
  boost::recursive_mutex::scoped_lock cr(configuration_mutex_);
  int cloud_index = -1;

  // Do we have the base->base_laser Tx yet?
  if(frame_to_cloud_.find(point_cloud->header.frame_id) == frame_to_cloud_.end())
  {
    ROS_DEBUG("Setting up laser %d (frame_id=%s)\n", (int)frame_to_cloud_.size(), point_cloud->header.frame_id.c_str());
    clouds_.push_back(new AMCLPointCloud(*cloud_));
    clouds_update_.push_back(true);
    cloud_index = frame_to_cloud_.size();

    tf::Stamped<tf::Pose> ident (tf::Transform(tf::createIdentityQuaternion(),
                                             tf::Vector3(0,0,0)),
                                 ros::Time(), point_cloud->header.frame_id);
    tf::Stamped<tf::Pose> cloud_pose;
    try
    {
      this->tf_->transformPose(base_frame_id_, ident, cloud_pose);
    }
    catch(tf::TransformException& e)
    {
    ...
    frame_to_cloud_[point_cloud->header.frame_id] = cloud_index;
  } else {
    // we have the laser pose, retrieve laser index
    cloud_index = frame_to_cloud_[point_cloud->header.frame_id];
  }
  ...
  if(pf_init_)
  {
    ...
    if(update)
      for(unsigned int i=0; i < clouds_update_.size(); i++)
        clouds_update_[i] = true;
  }
  ...
  else if(pf_init_ && clouds_update_[cloud_index])
  {
    //printf("pose\n");
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      if (tf_broadcast_ == true)
      {
        ...
        ros::Time transform_expiration = (point_cloud->header.stamp +
